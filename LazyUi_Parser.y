...
%right TK_FACT
%left TK_DOT  /* Add precedence for field access */

/* Define types for non-terminals */
%type <ast_node> programme declarations_fonctions declaration_fonction expression expression_arithmetique terme facteur instructions instruction instruction_retour declaration_variable affectation appel_fonction_expr arguments bloc instruction_print expression_comparison instruction_if instruction_while instruction_for instruction_try try_block catch_block array_initializer expression_list array_access jadwal_initializer jadwal_fields jadwal_field
...
instruction
    ...
    | instruction_if { debug_print("Reducing: if statement"); $$ = $1; }
    | instruction_while { debug_print("Reducing: while statement"); $$ = $1; }
    | instruction_for { debug_print("Reducing: for statement"); $$ = $1; }
    | instruction_try { debug_print("Reducing: try statement"); $$ = $1; }
    ;
...
declaration_variable
    ...
    | TK_RA9M TK_IDENTIFIANT TK_SEMICOLON
    {
        debug_print("Found variable declaration without initialization");
        $$ = create_var_declaration_node($2, TYPE_RA9M, NULL, yylineno);
    }
    | TK_LISTA TK_IDENTIFIANT TK_AFFECTATION array_initializer TK_SEMICOLON
    {
        debug_print("Found array declaration");
        $$ = create_var_declaration_node($2, TYPE_LISTA, $4, yylineno);
    }
    | TK_JADWAL TK_IDENTIFIANT TK_AFFECTATION jadwal_initializer TK_SEMICOLON
    {
        debug_print("Found jadwal declaration");
        $$ = create_var_declaration_node($2, TYPE_JADWAL, $4, yylineno);
    }
    ;
...
array_initializer
    ...
    | TK_LBRACKET expression_list TK_RBRACKET
    {
...
expression
    ...
    | expression TK_DOT TK_IDENTIFIANT
    {
        debug_print("Found field access expression");
        $$ = create_field_access_node($1, $3, yylineno);
    }
    | TK_NUMBER
    {
...
affectation
    ...
    | expression TK_DOT TK_IDENTIFIANT TK_AFFECTATION expression TK_SEMICOLON
    {
        debug_print("Found field assignment");
        ASTNode* field_access = create_field_access_node($1, $3, yylineno);
        $$ = create_assignment_node(field_access, $5, yylineno);
    }
    ;
...
expression_comparison
    ...
    | expression_arithmetique TK_EQUAL expression_arithmetique
    {
        debug_print("Found equal comparison");
        $$ = create_binary_op_node(OP_EQ, $1, $3, yylineno);
    }
    | expression_arithmetique TK_DIFF expression_arithmetique
    {
        debug_print("Found not equal comparison");
        $$ = create_binary_op_node(OP_NEQ, $1, $3, yylineno);
    }
    | expression_arithmetique TK_SUP expression_arithmetique
    {
        debug_print("Found greater than comparison");
        $$ = create_binary_op_node(OP_GT, $1, $3, yylineno);
    }
    | expression_arithmetique TK_INF expression_arithmetique
    {
        debug_print("Found less than comparison");
        $$ = create_binary_op_node(OP_LT, $1, $3, yylineno);
    }
    | expression_arithmetique TK_SUP_EQ expression_arithmetique
    {
        debug_print("Found greater than or equal comparison");
        $$ = create_binary_op_node(OP_GTE, $1, $3, yylineno);
    }
    | expression_arithmetique TK_INF_EQ expression_arithmetique
    {
        debug_print("Found less than or equal comparison");
        $$ = create_binary_op_node(OP_LTE, $1, $3, yylineno);
    }
    ;
...
instruction_if
    : TK_ILA TK_LPAREN expression TK_RPAREN bloc
    {
        debug_print("Found if statement");
        $$ = create_if_node($3, $5, NULL, yylineno);
    }
    | TK_ILA TK_LPAREN expression TK_RPAREN bloc TK_ILAMAKANCH bloc
    {
        debug_print("Found if-else statement");
        $$ = create_if_node($3, $5, $7, yylineno);
    }
    ;

instruction_while
    : TK_HITACH TK_LPAREN expression TK_RPAREN bloc
    {
        debug_print("Found while statement");
        $$ = create_while_node($3, $5, yylineno);
    }
    ;

instruction_for
    : TK_L TK_LPAREN expression TK_RPAREN bloc
    {
        ...
        $$ = create_for_node(init, $3, update, $5, yylineno);
    }
    ;
...
jadwal_initializer
    : TK_LBRACE jadwal_fields TK_RBRACE
    {
...
jadwal_fields
    ...
    | jadwal_fields TK_COMMA jadwal_field
    {
...
jadwal_field
    : TK_IDENTIFIANT TK_AFFECTATION expression
    {
        debug_print("Found jadwal field with value");
        ASTNode* field_node = create_field_access_node(NULL, $1, yylineno);
        $$ = create_assignment_node(field_node, $3, yylineno);
    }
    ;
